...
struct fileParams
{
  ...
  struct Vector* dataDefinitions; /**< holds structs with varParams, with posRow, posCol, posSep parameters */
  struct Vector* binaryDefinitions; /**< holds structs with varParams, especially with inputBinaryWidth, binaryOffset, binaryDatatype */
...
struct foundValue
{
  struct varParams*  associatedVarParams; /**< the corresponding varParams giving an insight about the values location */
  struct fileParams* associatedFileParams; /**< the corresponding fileParams hinting the associated filename */
  uint64_t associatedValue; /**< the value to be stored/transported */
};


static void processLine (struct fileParams* fileSpec, int* varParamsIndex, int curLineNumber, struct Vector* foundValuesVec, char* myLine);
static void tryAppendingValueToFoundValuesVec(struct fileParams* fileSpec, struct varParams* varSpec, struct Vector* foundValuesVec, char* foundStr);
...
/**
 * Takes two pointers to define a substring in a char sequence which it copies into newly allocated memory
 */
    ...
    for(int i = 0; i < fileParamsVector->length; ++i)
    {
      ...
      if(NULL != foundValuesVec)
      {
        for(int j = 0; j < foundValuesVec->length; ++j)
        {
          ...
          if(blobarray_append(logging_containers + curFound->associatedVarParams->id, curFound->associatedValue, wtime(), curFound->associatedVarParams->logDif, curFound->associatedVarParams->datatype))
          {
...
SCOREP_Metric_Plugin_MetricProperties* get_event_info(char* event_name)
{
  ...
  SCOREP_Metric_Plugin_MetricProperties* return_values = calloc(2, sizeof(SCOREP_Metric_Plugin_MetricProperties));
  ...
  if(NULL != fileSpec && 0 < fileSpec->dataDefinitions->length)
  {
    ...
    if(NULL != strDuplicate)
    {
      ...
      if(NULL != strDuplicate)
      {
        ...
        {
          case 2:
            log_error("Could not insert variable specification to counters. Ran out of memory.");
            break;
          case 1:
            log_error("NULL Pointer error while inserting variable specification to counters.");
            break;
          case 0:
            insertSuccessfull = 1;
            break;
          default:
            log_error("Some error appeared while inserting variable specification to counters.");
        }
  ...
  {
    log_error_string("Could not parse variable specification \"%s\". Syntax incorrect?", event_name);
  }
...
static int tryInsertingFileParams(struct fileParams* fileSpec)
{
  ...
  for(int i = 0; i < fileParamsVector->length; ++i)
  {
    ...
    if(0 == strcmp(curFileParams->filename, fileSpec->filename))
    {
      ...
      returnValue = tryInsertingVarParamsSorted(curFileParams, fileSpec->dataDefinitions->data[0]);
      vec_destroy(fileSpec->dataDefinitions);
      vec_destroy(fileSpec->binaryDefinitions);
      free(fileSpec->filename);
      free(fileSpec);
...
static int tryInsertingVarParamsSorted(struct fileParams* fileSpec, struct varParams* varSpec)
{
  ...
  if(0 < varSpec->inputBinaryWidth)
  {
    ...
    {
      int wasInserted = 0;
      /* run through all the elements in the vector looking for a varParams with a binaryOffset set greater than the one to be inserted,
       * then inserts the new varParams before that */
      for(int i = 0; i < (fileSpec->binaryDefinitions->length -1); ++i)
      {
        if(((struct varParams*)fileSpec->binaryDefinitions->data[i])->binaryOffset > varSpec->binaryOffset)
        {
          memmove(fileSpec->binaryDefinitions->data + i + 1, fileSpec->binaryDefinitions->data + i, sizeof(void*) * (fileSpec->binaryDefinitions->length - i));
      ...
      if(!wasInserted)
      {
        /* if no varParams was found greater than the new one, insert the new varParams at the end of the vector */
        fileSpec->binaryDefinitions->data[fileSpec->binaryDefinitions->length - 1] = varSpec;
        return 0;
      }
    }
  } else
  {
    ...
    {
      int wasInserted = 0;
      /* run through all the elements in the vector looking for a varParams with a posRow set greater than the one to be inserted,
       * then inserts the new varParams before that */
      for(int i = 0; i < (fileSpec->dataDefinitions->length -1); ++i)
      {
        if(((struct varParams*)fileSpec->dataDefinitions->data[i])->posRow > varSpec->posRow)
        {
          memmove(fileSpec->dataDefinitions->data + i + 1, fileSpec->dataDefinitions->data + i, sizeof(void*) * (fileSpec->dataDefinitions->length - i));
...
/**
 * is called after get_event_info to add the counters (which I do in get_event_info) in here it initializes the logging containers
 */
static int32_t add_counter(char* event_name)
{
  ...
  if(count_of_counters == calls_to_add_counter)
  {
    ...
    for(int i = 0; i < fileParamsVector->length; ++i)
    {
      ...
      {
        log_error_string("File \"%s\" can not be accessed for reading.", curFileSpec->filename);
      }
      ...
      if(NULL != foundValuesVec)
      {
        ...
        for(int j = 0; j < foundValuesVec->length; ++j)
        {
          ...
          struct foundValue* curFound = foundValuesVec->data[j];
          if(-1 < curFound->associatedVarParams->id && curFound->associatedVarParams->id < count_of_counters)
          {
            struct blob_holder* newLoggingHolder = blobarray_create(BLOBARRAY_INIT_BUF, curFound->associatedValue);
            if(NULL != newLoggingHolder)
            {
              memcpy(logging_containers + curFound->associatedVarParams->id, newLoggingHolder, sizeof(struct blob_holder));
              free(newLoggingHolder);
            } else
            {
              log_error("Could not allocate a few bytes of memory to create a swap blob_holder.");
            }
          } else
          {
  ...
  return calls_to_add_counter-1; //this return value defines the id with which the counter will be associated
}
/**
...
static uint64_t get_all_values(int32_t id, SCOREP_MetricTimeValuePair** time_value_list)
{
  ...
  {
    saved_nr_results = blobarray_get_TimeValuePairs(logging_containers + actualId, time_value_list);
...
static struct fileParams* parseVariableSpecification(char* specStr, int idToBeAssigned)
{
  ...
  {
    log_error_string("Could not allocate memory for vectors in struct fileParams. Input specStr was \"%s\".", specStr);
    free(parsedData);
  ...
  if(NULL != posOfCurDelim)
  {
    ...
    if(NULL == varSpec)
    {
      log_error("Ran out of memory when trying to allocate a few bytes for a varParams struct.");
      vec_destroy(parsedData->dataDefinitions);
      vec_destroy(parsedData->binaryDefinitions);
      free(parsedData);
      free(curVarName);
    ...
    {
      log_error("Ran out of memory when trying to insert a varParams struct into a vector of a fileParams struct.");
      vec_destroy(parsedData->dataDefinitions);
      vec_destroy(parsedData->binaryDefinitions);
      free(parsedData);
      free(curVarName);
...
static SCOREP_MetricValueType parseDatatype(char * curDatatypeName, int* inputHex, int* inputBinaryWidth, Fileparser_Binary_Datatype* binaryDatatype)
{
  ...
  } else if(0 == strcasecmp(curDatatypeName, "uint_hex"))
  {
    curDatatype = SCOREP_METRIC_VALUE_UINT64;
    *inputHex = 1;
  } else if(0 == strcasecmp(curDatatypeName, "float") || 0 == strcasecmp(curDatatypeName, "double"))
  {
...
/**
 * Parses a whole file according to parameters given with fileSpec and returns an array of found results
 * "Files öffnen und schliessen ist immer sehr teuer" (Robert Schöne)
 * DONE: declare the used variables as static, thereby they will be kept as global variables (within the file; even when I declare them within a method to be static)
 *       of course this also means I will have to allocate them at the beginning and have to free them at the end
 */
static struct Vector* parseWholeFile(struct fileParams* fileSpec)
{
  ...
  do {
    ...
    if(0 < readReturn)
    {
      ...
      {
        struct varParams* binaryVarSpec = fileSpec->binaryDefinitions->data[binaryParamsIndex];
        while (NULL != binaryVarSpec && binaryVarSpec->binaryOffset < curTotalBytesRead)
        {
          if(NULL != binaryOverlapBuf)
          {
            /* copy both the fragment of old bytes and freshly read bytes to binaryBytesForParsing */
            memcpy(binaryBytesForParsing, binaryOverlapBuf, curTotalBytesRead - readReturn - binaryVarSpec->binaryOffset);
            int byteCountStillToCopy = binaryVarSpec->inputBinaryWidth - (curTotalBytesRead - readReturn - binaryVarSpec->binaryOffset);
            memcpy(binaryBytesForParsing + byteCountStillToCopy, readBuf, byteCountStillToCopy);

            /* actually register the read bytes */
            tryAppendingValueToFoundValuesVec(fileSpec, binaryVarSpec, foundValuesVec, binaryBytesForParsing);
          ...
          } else
          {
            ...
            {
              if((binaryVarSpec->binaryOffset + binaryVarSpec->inputBinaryWidth) <= curTotalBytesRead)
              {
                /* copy the found bytes for appending them to foundValues later */
                memcpy(binaryBytesForParsing, readBuf + (binaryVarSpec->binaryOffset - (curTotalBytesRead - readReturn)), binaryVarSpec->inputBinaryWidth);
                tryAppendingValueToFoundValuesVec(fileSpec, binaryVarSpec, foundValuesVec, binaryBytesForParsing);
              } else {
                /* the current readBuf does not contain all bytes that we wish to read, utilize binaryOverlapBuf */
                binaryOverlapBuf = calloc(8, sizeof(char));
                if(NULL != binaryOverlapBuf)
                {
                  memcpy(binaryOverlapBuf, readBuf + (binaryVarSpec->binaryOffset - (curTotalBytesRead - readReturn)), curTotalBytesRead - binaryVarSpec->binaryOffset);
                } else {
                  log_error("Could not read binary value, allocation of 8 bytes failed.");
                }
                continue;
              }
            } else
            {
              log_error("This error should never appear. binaryVarSpec->binaryOffset is smaller than (curTotalBytesRead - readReturn).");
            }
          }
      ...
      } else
      {
        ...
        if(NULL != overlapBuf) {
          ...
          if(NULL == overlapSwapBuf)
          {
            log_error("Could not allocate memory for parsing the current line in a file.");
            free(overlapBuf);
            vec_destroy(foundValuesVec);
            return NULL;
          }
        ...
        {
          curNewlineIndex[0] = '\0';
          processLine(fileSpec, &varParamsIndex, curLineNumber, foundValuesVec, prevIndex);
          ++curLineNumber;
...
/**
 * Tries to get a file Descriptor for the file denoted in varParams, if a fileDescriptor is already there, seek to beginning of file
 */
static FILE* prepareFileDescriptorForParsing(struct fileParams* fileSpec)
{
...
/**
 * Part of parseWholeFile, just process a line, adding the values of found field matches to the result Vector foundValuesVec
 */
static void processLine (struct fileParams* fileSpec, int* varParamsIndex, int curLineNumber, struct Vector* foundValuesVec, char* myLine)
{
  ...
  struct varParams* curVarSpec = (struct varParams*)fileSpec->dataDefinitions->data[*varParamsIndex];
...
static void tryAppendingValueToFoundValuesVec(struct fileParams* fileSpec, struct varParams* varSpec, struct Vector* foundValuesVec, char* foundStr)
{
  ...
  if(NULL != found)
  {
    ...
    if(0 < varSpec->inputBinaryWidth) {
      found->associatedValue = parseValueBinary(foundStr, varSpec->inputBinaryWidth, varSpec->binaryDatatype);
    } else {
