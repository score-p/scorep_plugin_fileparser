# scorep_plugin_netstat
# Netstat Plugin
--
Netstat Plugin is a shared library to be used with Score-P. It is capable of logging any value available via standard FileDescriptors on a machine. I.e. one can log:
...
Just run `make`
Or if you like it more complicated use cmake
```
mkdir build
cd build
cmake ..
...
```
SCOREP_METRIC_NETSTATS_PLUGIN=<variable>[','<variable>]*
<variable> = [ <variablename> ':' ] [ <field-datatype> '@' ] <path-to-file> '+' <field-declaration>
<variablename> = any char except '\0' and ':'
...
<path-to-file> = path to the file, may not contain '+'
<field-declaration> = <field-parameter> [';'<field-parameter>]*
<field-parameter> = ('C' | 'c' | 'R' | 'r' | 'L' | 'l'  '=' <field-value> ) | ('S' | 's' = <field-separator>) | 'D' | 'd' | 'P' | 'p' | 'A' | 'a'
...
```
export SCOREP_METRIC_NETSTATS_PLUGIN="netstat InOctets:int@/proc/net/netstat+c=7;r=3;s= ;a;d"
```
...
Another example:
```
export SCOREP_METRIC_NETSTATS_PLUGIN="CPU Clock 1:double@/proc/cpuinfo+c=2;r=7;s= ;p,CPU Clock 2:double@/proc/cpuinfo+c=2;r=34;s= ;p"
```
...
Can it read binary with an offset i.e., via pread int(64|32|16|8)_bin, uint(64|32|16|8)_bin, (double|float)_bin?
Yes. Yes it can.
